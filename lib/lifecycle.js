...
class LifeCycle {
    constructor(config, driver, logger, values) {
        this.config = config;
        this.driver = driver;
        this.logger = logger;
        this.history = [];
        this.report = [];
        this.values = config.values || values || {};
        this.errors = [];
    }
    ...
    async axe(pageConfig, report) {
        ...
        errors = errors.filter(e => {
            ...
            let rePath = ignore.path && new RegExp(ignore.path);
            ...
            e.nodes = e.nodes.filter(node =>
                (!rePath || ! rePath.test(pageConfig.url.pathname)) &&
    ...
    async processPage(pageUrl) {
        if (!this.lastTimestamp) this.lastTimestamp = new Date();
        ...
        try {
            ...
            if (this.config.axe && pageConfig.axe && pageConfig.axe.run) {
