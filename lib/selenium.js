...
class WebDriver {
    ...
    async create() {
        ...
        await this.driver.manage().setTimeouts({
            implicit: this.config.elementTimeout || 0,
        });
        await this.driver.manage().setTimeouts({
            pageLoad: this.config.pageLoadTimeout || 60000,
        });
        await this.driver.manage().setTimeouts({
            script: this.config.scriptTimeout || 30000
        });
        await this.driver.manage().deleteAllCookies();
        if (this.config.maximise) await this.driver.manage().window().maximize();
    ...
    async axe(options) {
        ...
        return await new AxeBuilder(this.driver)
            .options(options)
            .analyze();
    }
